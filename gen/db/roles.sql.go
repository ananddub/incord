...
SELECT id, guild_id, name, color, position, created_at, deleted, updated_at FROM roles
...
func (q *Queries) GetEveryoneRole(ctx context.Context, guildID pgtype.UUID) (Role, error) {
	row := q.db.QueryRow(ctx, getEveryoneRole, guildID)
	var i Role
	err := row.Scan(
		&i.ID,
		&i.GuildID,
...
const listEveryoneAssignments = `-- name: ListEveryoneAssignments :many
SELECT
...
type ListEveryoneAssignmentsRow struct {
	GuildID pgtype.UUID `json:"guild_id"`
	RoleID  pgtype.UUID `json:"role_id"`
	UserID  pgtype.UUID `json:"user_id"`
}
...
func (q *Queries) ListEveryoneAssignments(ctx context.Context) ([]ListEveryoneAssignmentsRow, error) {
	rows, err := q.db.Query(ctx, listEveryoneAssignments)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListEveryoneAssignmentsRow{}
	for rows.Next() {
		var i ListEveryoneAssignmentsRow
		if err := rows.Scan(&i.GuildID, &i.RoleID, &i.UserID); err != nil {
			return nil, err
		}
